...
useEffect(() => {
  const fetchStats = async () => {
    try {
      ...
      const res = await fetch(
        "http://localhost:5000/api/employer-profile/dashboard-stats",
        {
          headers: {
            Authorization: `Bearer ${token}`
          }
    ...
    Manage jobs, applicants and your company profile
      ...
      <div className="bg-gradient-to-r from-[#7a0054] via-[#9c005e] to-[#d1004f] text-white p-6 rounded-xl mb-6">
        ...
        <div className="flex gap-3 mt-4 flex-wrap">
          ...
          <Badge
  label={isVerified ? "Verified Company" : "Not Verified"}
  color={isVerified ? "green" : "yellow"}
/>


     <Badge label={`Active Jobs: ${stats.activeJobs}/${planLimit}`} color="gray" />
...
<StatCard title="Pending Approval" value={stats.pendingJobs} />
